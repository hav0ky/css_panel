...
import type { DB_Count, SA_Admin } from '@/types/db/plugin'
import type { Flag } from '@/types/db/css'
import db from '@/lib/mysql'

const fields = ['id', 'player_steamid', 'player_name', 'flags', 'immunity', 'server_id', 'ends']
...
const Admins = {
	getAll: async (page: number = 1, limit: number = 100000): Promise<SA_Admin[]> => {
		try {
			...
			console.log(`[DB] Error while getting all admins: ${err}`)
			return []
		}
	},
	...
	create: async ({ id, player_steamid, player_name, flags, immunity, server_id }: SA_Admin): Promise<number | null> => {
		try {

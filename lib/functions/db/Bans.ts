...
import type { DB_Count, SA_Ban } from '@/types/db/plugin'
...
const Bans = {
	getAll: async (page: number, limit: number): Promise<SA_Ban[]> => {
		try {
			...
			console.log(`[DB] Error while getting all bans: ${err}`)
			return []
		}
	},
	...
	create: async (props: Partial<SA_Ban>): Promise<number | null> => {
		...
		try {
			...
			console.log(`[DB] Error while creating bans: ${err}`)
			return null
		}
	},
	update: async (id: number, props: Partial<SA_Ban>): Promise<boolean> => {
		try {
			...
			console.log(`[DB] Error while updating bans: ${err}`)
			return false
		}
	},
	delete: async (banId: number): Promise<boolean> => {
		try {
			...
			console.log(`[DB] Error while deleting ban: ${err}`)
			return false
		}
	},
	count: async (): Promise<number> => {
		try {
			...
			console.log(`[DB] Error while counting bans: ${err}`)
			return 0
		}
	},
}

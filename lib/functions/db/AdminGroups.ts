...
import type { DB_Count, SA_AdminGroup } from '@/types/db/plugin'
...
const Admins = {
	getAll: async (): Promise<SA_AdminGroup[]> => {
		try {
			...
			console.log(`[DB] Error while getting all admin groups: ${err}`)
			return []
		}
	},
	getById: async (groupId: string): Promise<SA_AdminGroup | null> => {
		try {
			...
			console.log(`[DB] Error while getting the admin group: ${err}`)
			return null
		}
	},
	create: async (props: SA_AdminGroup): Promise<number | null> => {
		try {
			...
			console.log(`[DB] Error while creating admin group: ${err}`)
			throw err
		}
	},
	update: async (id: string, props: Partial<SA_AdminGroup>): Promise<boolean> => {
		try {
			...
			console.log(`[DB] Error while updating admin group: ${err}`)
			throw err
		}
	},
	delete: async (groupId: string): Promise<boolean> => {
		try {
			...
			console.log(`[DB] Error while deleting admin group: ${err}`)
			throw err
		}
	},
	count: async (): Promise<number> => {
		try {
			...
			console.log(`[DB] Error while counting admin groups: ${err}`)
			return 0
		}
	},
}
